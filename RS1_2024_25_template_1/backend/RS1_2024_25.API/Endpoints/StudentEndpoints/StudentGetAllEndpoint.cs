    ...
    .WithResult<MyPagedList<StudentGetAllResponse>>
{
    ...
    public override async Task<MyPagedList<StudentGetAllResponse>> HandleAsync([FromQuery] StudentGetAllRequest request, CancellationToken cancellationToken = default)
    {
        ...
        {
            bool space = request.FirstLast.Contains(" ");
            if (space)
            {
                var split = request.FirstLast.Split(" ");
                query = query.Where(s => (s.User.FirstName.ToLower().Contains(split[0]) && s.User.FirstName.ToLower().Contains(split[1]))
                || (s.User.FirstName.ToLower().Contains(split[1]) && s.User.FirstName.ToLower().Contains(split[0])));
            }
            else
                query = query.Where(s => (s.User.FirstName.ToLower().Contains(request.FirstLast.ToLower()) || s.User.FirstName.ToLower().Contains(request.FirstLast.ToLower())));
        }
    ...
    public class StudentGetAllRequest : MyPagedRequest
    {
        ...
        public string? FirstLast {  get; set; }=string.Empty;
    }
